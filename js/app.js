    ...
    goals: [
        {
            ...
            step: 2000,
            color: "var(--streak-or)",
            reason: "🤖 Вам нужно больше двигаться — сидячий образ жизни",
            done: false
        },
        {
            icon: "💧",
            ...
            step: 0.5,
            color: "#2196F3",
            reason: "🤖 Вы пьёте мало воды, это замедляет метаболизм",
            done: false
        },
    ],
};
...
function getLeague(flames) {
    return LEAGUES.find(l => flames >= l.min && flames <= l.max) || LEAGUES[0];
}

function getNextLeague(flames) {
    const i = LEAGUES.findIndex(l => flames >= l.min && flames <= l.max);
    return i < LEAGUES.length - 1 ? LEAGUES[i + 1] : null;
}

function sameLeague(flamesA, flamesB) {
    return getLeague(flamesA).name === getLeague(flamesB).name;
}
...
const posts = [
    {
        ...
        userId: "1",
        time: "15 мин назад",
        text: "Утренняя пробежка 5 км по парку! Погода шикарная 🌤 Кто со мной завтра?",
        likes: 12,
        comments: 3,
        liked: false,
        tag: "Активность"
    },
    {
        ...
        userId: "5",
        time: "1 час назад",
        text: "Приготовила ПП-пиццу на цветной капусте! Всего 180 ккал на порцию 🍕",
        likes: 24,
        comments: 8,
        liked: true,
        tag: "Рецепт",
    ...
    },
    {
        ...
        userId: "4",
        time: "2 часа назад",
        text: "Официально минус 2 кг за неделю! Дневник питания реально помогает 💪",
        likes: 31,
        comments: 5,
        liked: false,
        tag: "Результат"
    },
    {
        ...
        userId: "3",
        time: "3 часа назад",
        text: "Утренняя йога — лучшее начало дня. 20 минут и заряд энергии ☀️",
        likes: 18,
        comments: 2,
        liked: false,
        tag: "Активность",
    ...
    },
    {
        ...
        userId: "6",
        time: "5 часов назад",
        text: "61 день подряд без пропусков! 🔥🔥🔥 Готовлюсь к марафону.",
        likes: 45,
        comments: 12,
        liked: true,
        tag: "Мотивация"
    },
    {
        ...
        userId: "2",
        time: "20 мин назад",
        text: "Сделал 500 шагов до обеда — уже не ноль! 💪",
        likes: 5,
        comments: 1,
        liked: false,
        tag: "Активность"
...
const tagColors = {
    "Активность": {bg: "#E3F6ED", text: "#1A9E5C"},
    "Рецепт": {bg: "#FFF3E0", text: "#E67E22"},
    "Результат": {bg: "#E8F0FE", text: "#1A73E8"},
    "Мотивация": {bg: "#FDE8E8", text: "#D93025"}
};
...
let aiChatMessages = [
    {
        ...
        html: "Привет! Я подскажу по питанию, активности и привычкам. Выберите тему ниже или напишите вопрос."
    },
];
...
function renderContent() {
    ...
    switch (currentTab) {
        case 'profile':
            ...
            break;
        case 'diary':
            c.innerHTML = renderDiary();
            break;
        case 'aichat':
            c.innerHTML = renderAIChat();
            break;
        case 'progress':
            c.innerHTML = renderProgress();
            break;
        case 'video':
            c.innerHTML = renderVideo();
            break;
        case 'community':
            c.innerHTML = currentDialog ? renderDialog() : renderCommunity();
            break;
    }
}
...
function renderDiary() {
    const items = [{icon: "🥗", title: "Питание", label: "0 из 1955 ккал"}, {
        icon: "💧",
        title: "Вода",
        label: "Выпито 0 из 2 л"
    }, {icon: "🏃", title: "Активность", label: "0 из 500 ккал"}, {icon: "😴", title: "Сон", label: null}];
    return `<div style="padding:0 16px 24px">
...
function renderProgress() {
    ...
    const seriesByMetric = {
        ...
        calories: {
            ...
            values: ps.calories,
            color: "#FF9800",
            targetText: "Дефицит: 1800 ккал/день",
            precision: 0
        },
        movement: {
            label: "Движение",
...
function renderVideo() {
    ...
    const vids = [{title: "Собака мордой вниз", dur: "00:13"}, {
        title: "Прыжки с махами рук",
        dur: "00:13"
    }, {title: "Приседание и подъем ног", dur: "00:13"}];
    return `<div style="padding:0 16px 24px">
...
function renderFeed() {
    ...
    const filters = [{id: "all", label: "Все"}, {id: "Активность", label: "Активность"}, {
        id: "Рецепт",
        label: "Рецепты"
    }, {id: "Мотивация", label: "Мотивация"}, {id: "Результат", label: "Результаты"}];
    ...
    ${filtered.map(p => {
        ...
        const u = isApi ? {
            ...
            avatarColor: p.author.avatar_color,
            flames: 0
        } : (p.userId === "me" ? {id: "me", ...user} : getFriend(p.userId));
        if (!u) return '';
        ...
        const timeText = isApi ? new Date(p.created_at).toLocaleString('ru-RU', {
            hour: '2-digit',
            minute: '2-digit',
            day: '2-digit',
            month: '2-digit'
...
function renderChatItems(query) {
    ...
    return chatList.filter(c => {
        ...
        const u = getFriend(c.userId);
        return `<div class="chat-item" onclick="openDialog('${c.userId}')">
    ...
    }).join('') || '<div style="padding:40px;text-align:center;color:var(--text2);font-size:14px">Ничего не найдено</div>';
}
